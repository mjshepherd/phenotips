...
var LiveTableFilter = Class.create({
  ...
  serializeFilters: function()
  {
    ...
    var result = "";
    var filters = this.filterNodes.invoke('select','input[name], select').flatten();
    for (var i=0;i<filters.length;i++) {
      // Ignore filters with blank value if are not used as filters yet
      if (!filters[i].value.blank() || this.filters[filters[i].name] &&
          [this.filters[filters[i].name]].flatten().collect(function(item) {return (!item || item.blank && item.blank()) ? undefined : item}).compact().size() > 0){
